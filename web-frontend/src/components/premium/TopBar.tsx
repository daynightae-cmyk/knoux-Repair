  ...
  return (
    <header className="knoux-topbar" style={{ height: '52px', WebkitAppRegion: 'drag' } as React.CSSProperties} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="flex items-center gap-3 px-4 h-full" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
        ...
        <button type="button" className="knoux-mcp-topbar-trigger" onClick={() => window.dispatchEvent(new CustomEvent('knoux:ai-open'))} title={isRtl ? 'فتح KNOUX AI' : 'Open KNOUX AI'} aria-label={isRtl ? 'فتح KNOUX AI' : 'Open KNOUX AI'}>
          <BrainCircuit size={15}/><span>KNOUX AI</span><i aria-hidden="true"/>
        </button>
