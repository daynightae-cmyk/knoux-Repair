...
async function navigateEvidenceRoute(page, url, label) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      console.warn(`[vitality-evidence] ${label}: navigation lifecycle timeout on attempt ${attempt}; committed=${committed}; readyState=${readyState}`);
      if (committed) return;
...
async function openRoute(page, service, selector) {
  const url = `${ORIGIN}/?view=vitality&nosplash=1&service=${encodeURIComponent(service)}`;
