...
$Session = Start-KnouxSession -ToolId 'SM04' -ToolName 'Scan System Image' -Category '01-System-Maintenance' -RiskLevel 'READ_ONLY' -Mode $(if ($AnalyzeOnly) { 'analyze' } elseif ($WhatIf) { 'preview' } else { 'run' })
...
if (-not ($AnalyzeOnly -or $WhatIf) -and $Session.RequiresAdmin -and -not (Test-KnouxAdministrator)) {
    ...
    if (Confirm-KnouxAction 'Proceed with DISM ScanHealth?') {
        ...
        $run = Invoke-KnouxNativeCommand -FilePath "$env:SystemRoot\System32\Dism.exe" -ArgumentList @('/English', '/Online', '/Cleanup-Image', '/ScanHealth') -TimeoutSeconds 2400
        if (-not $run) {
            $Session.Status = 'Failed'
