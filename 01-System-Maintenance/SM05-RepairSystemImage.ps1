        ...
        if ($Session.Status -ne 'Failed') {
            ...
            if ($needsRepair -or (Confirm-KnouxAction 'Run DISM RestoreHealth to repair any corruption?')) {
                ...
                if (-not [System.Console]::IsInputRedirected) {
                    $src = Read-Host 'Local source: path to install.wim/install.esd (blank = Windows Update)'
                    if ($src.Trim()) {
                        if (-not (Test-Path -LiteralPath $src.Trim())) {
                            ...
                            if (-not $info) {
                                ...
                                if ($indexes.Count -eq 0) {
                                    ...
                                    $selIdx = Read-Host ('Select image index [{0}]' -f $(if ($matched) { $matched.Index } else { $indexes[0].Index }))
